...
export default function Browser() {
    ...
    const [response, setResponse] = useState({result: {messageHeaders: []}})
    ...
    const getFormattedDate = (response) => {
        if (response.result.messageHeaders !== undefined) {
            const receivedHeader = response.result.messageHeaders.headers.find(el => el.name.toUpperCase() === "X-RECEIVED");
            const date = receivedHeader ? receivedHeader.value.split(";")[1].trim() : "";
            let formattedDate = getFormattedDate(date, {date: this.props.data.internalDate, parserFn: parseInt});
