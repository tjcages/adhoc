...
export class Navigation extends React.Component {
  ...
  renderMessages() {
    ...
    console.log('messagesize: ' + JSON.stringify(messages.length()))
    // this.props.messagesResult.messages.map((message) => return (
    //   message.id = id
    // ))
    ...
    return (
      labels.map((label) => { 
        ...
        if (messages.length > 0) {
          return (
            <div key={label}>
              <h3 className="navigation-list-header">{ label.toLowerCase().charAt(0).toUpperCase() + label.toLowerCase().slice(1) }</h3>
              {
